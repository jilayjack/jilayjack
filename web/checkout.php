...
$result=$obj->getcheckout($email);
			...
			{
                $cid=$row["fk_product_id"];
                //echo $cid;
                $res=$obj->getproduct($cid);

                //$id=$row["fk_product_id"];
				$_row=$res->fetch_assoc();
		
        echo '	 <div class="cart-header2">';
		echo '		 <a class="close2" href="cartdelete.php?id='.$_row["pk_product_id"].'"> </a>';
		echo '		  <div class="cart-sec simpleCart_shelfItem">';
		echo '				<div class="cart-item cyc">';
		echo '					 <img src="'.$_row["product_image1"].'" class="img-responsive"  alt=""/>';
		echo '				</div>';
		echo '			   <div class="cart-item-info">';
		echo '				<h3><a href="#">'.$_row["product_name"].'</a><br><span>Model No: '.$_row["pk_product_id"].'</span></h3>';
		echo '				<span>Price: '.$_row["product_prize"].'</span>';
		$total=$total+$_row["product_prize"];
		//echo '				<ul class="qty">';
		...
		echo '	  </div>';
		  ...
		  <div class="col-md-3 cart-total">
			 <a class="continue" href="userside.php">Continue to basket</a>
			 ...
			 <div class="clearfix"></div>
			 <a class="order" href="insertbill.php?email=<?php echo $email?>">Place Order</a>
